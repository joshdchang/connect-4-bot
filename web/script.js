...
tf.loadLayersModel('../modelLogic/model.json').then(m => {
    model = m;
    if (humanPlayer !== game.nextPlayer) {
        predict(game.board, game.nextPlayer).then(outputs => {
            game.moveAuto(outputs, true)
            displayBoard()
        })
    }
});

async function predict(inputs, nextPlayer) {
    let input = [...inputs]
    input.push(nextPlayer)
    const prediction = await model.predict(tf.tensor([input])).array()
    return prediction[0]
}

async function move(location) {
    game.move(location % 7, true)
    displayBoard()
    if (!game.isOver && game.nextPlayer !== humanPlayer) {
        let prediction = await predict(game.board, game.nextPlayer)
        game.moveAuto(prediction, true)
    }
    if (game.isOver) {
        if (humanPlayer === game.winner) {
            $('#info').text('You won! Click to reset.')
        }
        if (-humanPlayer === game.winner) {
            $('#info').text('The model won! Click to reset.')
        }
        if (game.winner === 0) {
            $('#info').text('A tie! Click to reset.')
        }
    }
    displayBoard()
}
...
function displayBoard() {
    for (var i = 0; i < game.board.length; i++) {
        $(`td.${i}`).css('background', (game.board[i] === -1) ? '#fdd835' : ((game.board[i] === 0) ? 'white' : '#f44336'))
    }
}
...
$('td').click(function () {
    if (game.isOver) {
        game.reset()
        $('#info').text('Click to place a tile.')
        displayBoard()
        ...
        let location = parseInt(this.className)
        if (game.nextPlayer === humanPlayer && !game.isOver) {
            move(location)
        }
    }
});